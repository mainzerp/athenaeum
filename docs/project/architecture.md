...
- **WebUI** — FastAPI + Jinja2 templates + htmx (+ Alpine.js for small interactions), with **3d-force-graph** (vendored under `webui/static/vendor`, WebGL/three.js) for the trace replay graph. No Node toolchain; htmx loads from CDN, the graph stack is vendored.
...
4. `/api/graph` scans markdown links across the user's bundle into nodes/folders/edges JSON for the 3D relations universe (`color` from trust/staleness, `group` from `type`, `folder`/`depth`/`kind` from the path, tooltip from tags); folder nodes are emitted for arbitrary depth (depth 1 = star, depth >= 2 = planet), documents are moons at any depth. The graph page itself renders the flat `/api/graph/universe` payload as a 2D sunburst on a plain canvas (no WebGL): root-level documents form a ring of dots around a glowing center anchor, each top-level folder is a sector docking outward (angle proportional to file count, subfolder angular sub-grouping via `parent_folder`), every document is exactly one dot whose radial position is its sqrt-scaled link_density, and the payload `edges` render as thin center-bowed arcs that highlight with their endpoints on hover/selection. Clicks drive an animated 2D zoom (dot / sector / center ring; Esc or Fit view resets to the overview), and a minimap mirrors the scene plus the current viewport. The legacy `/api/graph` payload feeds the ForceGraph3D detail/trace views (trace replay), which place nodes with the default d3 force layout. Folder nodes render as bodies (star: additive glow sprite, planet: matte `MeshLambertMaterial` sphere) at fixed sizes (star > planet), with the SpriteText label offset above the body.
...
Two further dot-dir stores record what arrives at the library boundary; both are invisible to every OKF `.md` traversal and never snapshotted (same posture as `.traces`).
