...
- **WebUI** — FastAPI + Jinja2 templates + htmx (+ Alpine.js for small interactions); the graph page and the trace replay graph render as a 2D sunburst on a plain canvas (no WebGL). No Node toolchain; htmx is vendored at `static/htmx.min.js` (no CDN dependency, 0.29.0). Document bodies render **server-side** via markdown-it-py (GFM tables/strikethrough + tasklists plugin, Pygments syntax highlighting, raw HTML escaped — no sanitizer, no marked/DOMPurify CDN).
...
5. The librarian runs its agent loop under the per-user run gate (§7 — a second same-kind run for the same user is rejected with a retryable busy error): system prompt + user intent -> LLM -> tool calls against the internal 9-tool view -> `LibraryBackend` -> repeat until a final text response or the `max_iterations` cap (default 10, per provider connection; one iteration is one assistant response — a response carrying several tool calls counts once, and the prompts pin batching independent calls; on cap exhaustion, request a final answer with no tools). Every internal dispatch runs its backend call on a worker thread (`asyncio.to_thread`, §7), so library I/O never blocks the event loop. Each internal dispatch is recorded into the trace session (operation, summarized arguments/result, duration, errors); LLM token usage is summed across completions. Request answers additionally pass a deterministic hygiene check (raw tool-call JSON / process narration) with one no-tools re-ask; an unresolved retry is returned with a deterministic post-run note, never dropped.
...
`LibraryBackend` implements the full OKF operation catalog. The librarian LLM sees a curated view of exactly 11 tools — 10 dispatched 1:1 onto the backend plus `run_computation`, which routes to the shared `ComputationRunner` (the curator LLM sees the same view minus `run_computation`):

| LLM tool | Purpose |
|---|---|
| `list_dir` | list a directory |
| `read_document` | read frontmatter + body (a suffix-less path retries once as `<path>.md`; a directory path errors with "is a directory, use list_dir") |
...
- **LibrarianManager cache** — all cache reads/mutations (`get`/`evict`/`evict_idle`) hold the manager lock: callers run on the event loop and the WebUI threadpool, so an unguarded evict could race a concurrent `get`. Eviction (explicit, idle, or `close()` at app shutdown) cancels both agents' pending embed-reconcile tasks.
