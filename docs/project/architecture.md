...
4. `/api/graph` scans markdown links across the user's bundle into nodes/folders/edges JSON for the 3D relations universe (`color` from trust/staleness, `group` from `type`, `folder`/`depth`/`kind` from the path, tooltip from tags); folder nodes are emitted for arbitrary depth (depth 1 = galaxy, depth >= 2 = system). The WebUI renders it with the vendored 3d-force-graph, but pins every body to a deterministic orbit position (folder depth = radial distance; wikilinked siblings share adjacent orbit slots) instead of running a force simulation.
