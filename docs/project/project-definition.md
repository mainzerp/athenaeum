...
- **Agents** — per-user, in-process LLM agents that are the *only writers* to the bundle: the **librarian** (answers knowledge requests, stores/updates knowledge) and the **curator** (`athenaeum.curator`, maintenance/curation passes). They expose 7 intent-based MCP tools to the outside; the librarian drives an 11-tool internal view over the library backend, the curator the same view minus `run_computation`.
...
- **Scheduled curation** (0.11.0) — optional per-user nightly `library_maintain` + `library_curate` run at a fixed UTC time, configured in the WebUI (curator tab, per-user opt-out), journaled in Activity. The curator tab shows the schedule state and the last run timestamp (0.23.0).
